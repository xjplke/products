...
License:   GPL
Group:     System
Requires:  python,python-setuptools,mysql,mysql-devel,mysql-server,java-1.7.0-openjdk,freeradius,freeradius-mysql,freeradius-utils,nginx,curl,vsftpd,db4,db4-utils
...
tar -xf %{SOURCE7} -C $RPM_BUILD_ROOT%{userpath}
...
#config ftp
echo "config ftp"
cp -r %{userpath}/vsftpdconf/* /etc/vsftpd/
cp -r %{userpath}/vsftpdconf/vsftpd.rpmsave /etc/pam.d/vsftpd
